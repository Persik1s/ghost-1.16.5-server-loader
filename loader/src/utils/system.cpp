...
std::string System::Web::Get(const char* ip,const char* name,int port){
	...
	*/
	...
	//Connect in Addres 
	bool conn = connect(socket_client,(struct sockaddr*)&addres_client,sizeof(addres_client));
...
bool System::Process::Inecjet(std::string namePrcess){
	PROCESSENTRY32 entry = {sizeof(PROCESSENTRY32)};
	HANDLE snapProcess = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS,0);

	if(!Process32First(snapProcess,&entry)){
		return 0;
	}

	while(Process32Next(snapProcess,&entry)){
		if(strstr(entry.szExeFile,namePrcess.c_str())){
			HANDLE process = OpenProcess(PROCESS_ALL_ACCESS,0,entry.th32ProcessID);
			
			ManualMapDll(process,dll,sizeof(dll));
...
void System::File::CFile(const char* name){
	FILE* file = fopen(name,"w");
	fclose(file);
}
bool System::File::WriteString(const char* name,const char* string){
	FILE* file = fopen(name,"r+");
	if(!file)
		return 0;

	size_t count = fwrite(string,strlen(string),1,file);
	fclose(file);
	return 1;
}