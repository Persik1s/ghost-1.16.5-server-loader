	...
	if (ClearNonNeededSections) {
		...
		for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
			if (pSectionHeader->Misc.VirtualSize) {
				if ((SEHExceptionSupport ? 0 : strcmp((char*)pSectionHeader->Name, xorstr(".pdata")) == 0) ||
					strcmp((char*)pSectionHeader->Name, xorstr(".rsrc")) == 0 ||
					strcmp((char*)pSectionHeader->Name, xorstr(".reloc")) == 0) {
...
void __stdcall Shellcode(MANUAL_MAPPING_DATA* pData) {
	...
	if (pData->SEHSupport) {
		...
		if (excep.Size) {
			if (!_RtlAddFunctionTable(reinterpret_cast<PRUNTIME_FUNCTION>(pBase + excep.VirtualAddress), excep.Size / sizeof(IMAGE_RUNTIME_FUNCTION_ENTRY), (DWORD64)pBase)) {
